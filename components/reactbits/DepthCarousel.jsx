...
const DepthCarousel = ({
  ...
  // Synced after render (not during it) so the imperative layout always reads current props.
  useLayoutEffect(() => {
  ...
  return (
    ...
    >
      <div className="depth-carousel__stage" ref={stageRef}>
        {data.map((item, i) => (
          <div
            ...
            aria-label={`${i + 1} de ${count}${data[i].alt ? `: ${data[i].alt}` : ''}`}
      ...
      {showIndicators && count > 1 && (
        <div className="depth-carousel__dots" role="tablist" aria-label="Elegir proyecto">
          {data.map((_, i) => (
            <button
              key={i}
              type="button"
              ...
              aria-label={`Ver ${data[i].alt || `proyecto ${i + 1}`}`}
